...
(function (global) {
  ...
  function syncRiskFindings() {
    var Risk = global.OMSRisk;
    if (!Risk) return;
    var existingRefIds = {};
    ws.findings.forEach(function (f) { if (f.sourceRefId) existingRefIds[f.sourceRefId] = true; });
    Risk.store.list().forEach(function (rItem) {
      (rItem.data.findings || []).forEach(function (rf) {
        if (existingRefIds[rf.id]) return;
        WB.addItem(ws, 'findings', {
          title: rf.type, message: rf.message, sourceType: 'risk', sourceLabel: rItem.name, confidenceStatus: 'Observed',
          relatedRiskModel: { modelId: rItem.id, modelName: rItem.name },
          relatedLayer: '', evidenceNeeded: rf.why || '', systemsInvolved: '', recommendedInvestigation: '',
          date: rf.savedAt, status: 'New', sourceRefId: rf.id
        });
        existingRefIds[rf.id] = true;
  ...
  function riskChip(rel) {
    if (!rel) return '';
    var Risk = global.OMSRisk;
    var live = rel.modelId && Risk ? Risk.store.get(rel.modelId) : null;
    var label = rel.modelName || (live && live.name);
    if (!label) return '';
    return live
      ? '<a class="badge badge--outline" href="risk.html?model=' + encodeURIComponent(rel.modelId) + '" title="Open in Operational Risk">Risk: ' + esc(label) + ' &rarr;</a>'
      : '<span class="badge badge--outline" title="Load this Risk Model to open it">Risk: ' + esc(label) + '</span>';
  }

  function syncResilienceFindings() {
    var Res = global.OMSResilience;
    if (!Res) return;
    var existingRefIds = {};
    ws.findings.forEach(function (f) { if (f.sourceRefId) existingRefIds[f.sourceRefId] = true; });
    Res.store.list().forEach(function (rItem) {
      (rItem.data.findings || []).forEach(function (rf) {
        if (existingRefIds[rf.id]) return;
        WB.addItem(ws, 'findings', {
          title: rf.type, message: rf.message, sourceType: 'resilience', sourceLabel: rItem.name, confidenceStatus: 'Observed',
          relatedResilienceModel: { modelId: rItem.id, modelName: rItem.name },
          relatedLayer: '', evidenceNeeded: rf.why || '', systemsInvolved: '', recommendedInvestigation: '',
          date: rf.savedAt, status: 'New', sourceRefId: rf.id
        });
        existingRefIds[rf.id] = true;
  ...
  function resilienceChip(rel) {
    if (!rel) return '';
    var Res = global.OMSResilience;
    var live = rel.modelId && Res ? Res.store.get(rel.modelId) : null;
    var label = rel.modelName || (live && live.name);
    if (!label) return '';
    return live
      ? '<a class="badge badge--outline" href="resilience.html?model=' + encodeURIComponent(rel.modelId) + '" title="Open in Resilience Intelligence">Resilience: ' + esc(label) + ' &rarr;</a>'
  ...
  function syncScaleFindings() {
    var Scale = global.OMSScale;
    if (!Scale) return;
    var existingRefIds = {};
    ws.findings.forEach(function (f) { if (f.sourceRefId) existingRefIds[f.sourceRefId] = true; });
    Scale.store.list().forEach(function (sItem) {
      (sItem.data.findings || []).forEach(function (sf) {
        if (existingRefIds[sf.id]) return;
        WB.addItem(ws, 'findings', {
          title: sf.type, message: sf.message, sourceType: 'scale', sourceLabel: sItem.name, confidenceStatus: 'Observed',
          relatedScaleAssessment: { modelId: sItem.id, modelName: sItem.name },
          relatedLayer: '', evidenceNeeded: sf.why || '', systemsInvolved: '', recommendedInvestigation: '',
          date: sf.savedAt, status: 'New', sourceRefId: sf.id
        });
        existingRefIds[sf.id] = true;
      });
    });
  }

  function scaleChip(rel) {
    if (!rel) return '';
    var Scale = global.OMSScale;
    var live = rel.modelId && Scale ? Scale.store.get(rel.modelId) : null;
    var label = rel.modelName || (live && live.name);
    if (!label) return '';
    return live
      ? '<a class="badge badge--outline" href="scale-readiness.html?model=' + encodeURIComponent(rel.modelId) + '" title="Open in Scale Readiness">Scale: ' + esc(label) + ' &rarr;</a>'
