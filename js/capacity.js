...
(function (global) {
  ...
  function operationalRiskHref(capacityModelId) {
    var base = global.OMSData ? global.OMSData.href('pages/risk.html') : 'risk.html';
    return base + '?fromCapacity=' + encodeURIComponent(capacityModelId);
  }

  function resilienceHref(capacityModelId) {
    var base = global.OMSData ? global.OMSData.href('pages/resilience.html') : 'resilience.html';
    return base + '?withCapacity=' + encodeURIComponent(capacityModelId);
  }

  function operatingDebtHref() {
    return global.OMSData ? global.OMSData.href('pages/operating-debt.html') : 'operating-debt.html';
  }

  function scaleReadinessHref() {
    return global.OMSData ? global.OMSData.href('pages/scale-readiness.html') : 'scale-readiness.html';
  }
  ...
  function renderDetailedSummary(mount) {
    ...
    mount.innerHTML =
      ...
      '</div>' +
      '<div class="hero__actions" style="margin-top:var(--space-5)"><button type="button" class="btn btn--secondary" id="cap-export-btn">Export JSON</button><button type="button" class="btn btn--secondary" id="cap-print-btn">Print / Save As PDF</button><a class="btn btn--secondary" href="' + operationalHealthHref('importCap', project.id) + '">Send To Health Model</a><a class="btn btn--secondary" href="' + operatingRhythmHref(project.id) + '">Add Capacity Review Rhythm</a><a class="btn btn--secondary" href="' + operationalRiskHref(project.id) + '">Map Operational Risk</a><a class="btn btn--secondary" href="' + resilienceHref(project.id) + '">Analyze Resilience</a><a class="btn btn--secondary" href="' + operatingDebtHref() + '">Review Operating Debt</a><a class="btn btn--secondary" href="' + scaleReadinessHref() + '">Check Scale Readiness</a></div>';
